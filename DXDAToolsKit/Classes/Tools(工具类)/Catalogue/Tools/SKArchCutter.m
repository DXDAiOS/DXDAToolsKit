...
+(instancetype)shareCut {
    static SKArchCutter *sington = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sington = [[self alloc] init];
    });
    return sington;
}

+(void)cuttingWithObject:(id)object direction:(UIRectCorner)direction cornerRadii:(CGFloat)cornerRadii {
    [[SKArchCutter shareCut] cuttingWithObject:object direction:direction cornerRadii:cornerRadii];
}
